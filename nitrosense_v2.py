#!/usr/bin/env python3
"""
NitroSense-like App V2 for Acer Nitro V16 (Linux)
- GUI + CLI
- CPU/GPU temps
- Fan speed control
- Performance modes
...
def set_performance_mode(mode):
    # mode: silent, balanced, performance
    # contoh implementasi: gunakan TLP / powerprofilesctl
...
class NitroSenseApp(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Linux NitroSense V16 - V2")
        self.resize(400, 300)

        # Labels
        self.info_label = QtWidgets.QLabel("Loading system info...")
        ...
        # Performance buttons
        self.silent_btn = QtWidgets.QPushButton("Silent Mode")
        self.balanced_btn = QtWidgets.QPushButton("Balanced Mode")
        self.performance_btn = QtWidgets.QPushButton("Performance Mode")
...
def main():
    parser = argparse.ArgumentParser(description="NitroSense V2 CLI/GUI")
    ...
    parser.add_argument('--mode', type=str, help='Set performance mode: silent/balanced/performance')
    parser.add_argument('--fan-set', type=int, help='Set fan speed manually (RPM)')
    ...
    if args.mode:
        if set_performance_mode(args.mode):
            print(f"Performance mode set to {args.mode}")
        else:
            print(f"Failed to set performance mode: {args.mode}")
    if args.fan_set:
        if set_fan_speed(args.fan_set):
            print(f"Fan speed set to {args.fan_set} RPM")
        else:
            print("Failed to set fan speed (requires root)")
